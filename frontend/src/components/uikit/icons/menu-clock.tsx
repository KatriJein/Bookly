interface IconProps {
    className?: string;
}

export function MenuClock({ className }: IconProps) {
    return (
        <svg
            className={className}
            width='20'
            height='20'
            viewBox='0 0 20 20'
            fill='none'
            xmlns='http://www.w3.org/2000/svg'
        >
            <path
                d='M10 18.75C5.175 18.75 1.25 14.825 1.25 10C1.25 5.175 5.175 1.25 10 1.25C14.825 1.25 18.75 5.175 18.75 10C18.75 14.825 14.825 18.75 10 18.75ZM10 2.5C5.8625 2.5 2.5 5.8625 2.5 10C2.5 14.1375 5.8625 17.5 10 17.5C14.1375 17.5 17.5 14.1375 17.5 10C17.5 5.8625 14.1375 2.5 10 2.5Z'
                fill='currentColor'
            />
            <path
                d='M12.5 13.125C12.3875 13.125 12.275 13.1 12.175 13.0375L9.05001 11.1625C8.95787 11.1064 8.88182 11.0274 8.82925 10.9332C8.77668 10.8391 8.74938 10.7329 8.75001 10.625V5.625C8.75001 5.275 9.02501 5 9.37501 5C9.72501 5 10 5.275 10 5.625V10.275L12.825 11.9625C12.9414 12.0338 13.0313 12.1411 13.0812 12.2682C13.1311 12.3952 13.1382 12.535 13.1014 12.6665C13.0647 12.7979 12.9861 12.9138 12.8776 12.9965C12.769 13.0793 12.6365 13.1244 12.5 13.125Z'
                fill='currentColor'
            />
        </svg>
    );
}
