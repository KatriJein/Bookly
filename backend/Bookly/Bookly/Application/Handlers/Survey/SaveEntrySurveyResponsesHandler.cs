...
{
    private const double SuggestedInitialPreferenceWeight = 0.6;
    ...
    public async Task<Result> Handle(SaveEntrySurveyResponsesCommand request, CancellationToken cancellationToken)
    {
        ...
        var formedPreferences = await FormPreferences(request.EntrySurveyDataDto.UserId, combinedGenresHashset, combinedAuthorsHashset,
             favoriteGenresHashset, hatedGenresHashset, favoriteAuthorsHashset, hatedAuthorsHashset, request.EntrySurveyDataDto.HatedGenresToBlacklist, cancellationToken);
    ...
    private async Task<CreatedPreferences> FormPreferences(Guid userId, HashSet<string> combinedGenres, HashSet<string> combinedAuthors,
        ...
        HashSet<string> hatedAuthorsHashset, bool strictBlackList, CancellationToken cancellationToken)
    {
        ...
        var genrePreferences = allRequiredGenres
            .Select(g => EntityToPreferenceDtoWithExcludingFromHashSets(userId, g.Id, g.Name, favoriteGenresHashset, hatedGenresHashset, strictBlackList))
            .ToHashSet();
        var authorPreferences = allRequiredAuthors
            .Select(a => EntityToPreferenceDtoWithExcludingFromHashSets(userId, a.Id, a.Name, favoriteAuthorsHashset, hatedAuthorsHashset, strictBlackList))
            .ToHashSet();
    ...
    private UserPreferenceDto EntityToPreferenceDtoWithExcludingFromHashSets(Guid userId, Guid entityId, string entityName, HashSet<string> favoritesHashset,
        HashSet<string> hatedHashset, bool strictBlackList)
    {
        ...
        var weight = preferenceType == PreferenceType.Liked
            ? SuggestedInitialPreferenceWeight
            : strictBlackList ? Const.AbsolutelyHatedGenreWeight : -SuggestedInitialPreferenceWeight;
        return new UserPreferenceDto(userId, entityId, preferenceType, weight);
    }
}
